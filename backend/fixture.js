...
const generateFakeUserDetail = async (userId) => {
  const userDetail = new UserDetail({
    ...
    gender: faker.helpers.arrayElement(['ប្រុស', 'ស្រី']),
    dob: faker.date.past({ years: 40 }),
    date_of_starting_work: faker.date.past({ years: 10 }),
    // date_of_starting_work: faker.date_between(start_date='-10y', end_date='today').strftime("%d/%m/%Y"),
    location_kh: faker.location.city(),
    location_type: faker.helpers.arrayElement(['ថ្នាក់ជាតិ', 'ក្រោមជាតិ']),
